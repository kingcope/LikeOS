...
void __putchar(char c, int x2, int y2, unsigned long *offscreen) {
	...
	for (y=0; y<FONT_HEIGHT; y++) {
		...
		for (x=0; x<FONT_WIDTH; x++) {
			...
			r=font[j+(((FONT_WIDTH*i+i/16))*3)];
			g=font[j+(((FONT_WIDTH*i+i/16))*3)+1];
			b=font[j+(((FONT_WIDTH*i+i/16))*3)+2];
...
void putcharc(char c, int x2, int y2, unsigned char r, unsigned char g, unsigned char b, unsigned long *offscreen) {
	...
	for (y=0; y<FONT_HEIGHT; y++) {
		...
		for (x=0; x<FONT_WIDTH; x++) {
			if (x+x2 > SCREENWIDTH || x+x2<0) continue;
			r2=font[j+(((FONT_WIDTH*i+i/16))*3)];
			g2=font[j+(((FONT_WIDTH*i+i/16))*3)+1];
			b2=font[j+(((FONT_WIDTH*i+i/16))*3)+2];
			
			j+=3;
			if ((r2==0) && (g2==0) && (b2==0))
				offscreen[SCREENWIDTH*(y+y2)+(x+x2)] = (r2<<16) | (g2<<8) | b2;
		}	
	}
}
...
void font_init() {
	font=(unsigned char*) kalloc(cdiso_getfilesize("FONT.RAW"));
	cdiso_readfile("FONT.RAW", font);
}
