...
void guiroot::init() {
	...
	windowicon=(unsigned char*) kalloc(cdiso_getfilesize("WND1.RAW"));
	cdiso_readfile("WND1.RAW", windowicon);	
	closeicon=(unsigned char*) kalloc(cdiso_getfilesize("CLOSE.RAW"));
	cdiso_readfile("CLOSE.RAW", closeicon);	
	maxicon=(unsigned char*) kalloc(cdiso_getfilesize("MAX.RAW"));
	cdiso_readfile("MAX.RAW", maxicon);	
	minicon=(unsigned char*) kalloc(cdiso_getfilesize("MIN.RAW"));
	...
	offscreen=(unsigned long*)kalloc(SCREENWIDTH*SCREENHEIGHT*3);
}
...
void guiroot::drawWallpaper() {
		...
		i=0;
		for (x=0; x<SCREENWIDTH*SCREENWIDTH; x++) {
			r=wallpaper[i];
			g=wallpaper[i+1];
			b=wallpaper[i+2];
			i+=3;		
...
void button::Draw(int x2, int y2)
{
	...
	if (buttondown == 0) {
		...
		/*
		root.DrawHorizLine(x2+x,y2+y,width,255,255,255);
		root.DrawVertiLine(x2+x,y2+y,height,255,255,255);
		root.DrawHorizLine(x2+x+1,y2+y+height-1,width-1,128,128,128);
		root.DrawHorizLine(x2+x,y2+y+height,width+1,64,64,64);
		root.DrawVertiLine(x2+x+width,y2+y,height,64,64,64);
		root.DrawVertiLine(x2+x+width-1,y2+y+1,height-1,128,128,128);
		*/
	} else {
		int i;
		for (i=0; i<height; i++) {
		...
		/*
		root.DrawHorizLine(x2+x,y2+y,width,0,0,0);
		root.DrawVertiLine(x2+x,y2+y,height,0,0,0);
		root.DrawHorizLine(x2+x+1,y2+y+1,width-1,128,128,128);
		root.DrawVertiLine(x2+x+1,y2+y+1,height-1,128,128,128);
		root.DrawHorizLine(x2+x,y2+y+height,width,0,0,0);
		root.DrawHorizLine(x2+x+1,y2+y+height-1,width-1,128,128,128);
		root.DrawVertiLine(x2+x+width,y2+y,height+1,0,0,0);
		root.DrawVertiLine(x2+x+width-1,y2+y+1,height-1,128,128,128);
		*/
	}
...
void textbox::Draw(int x2, int y2) {
	...
	/*
	root.DrawHorizLine(x+x2, y+y2, width, 128, 128, 128);
	root.DrawHorizLine(x+x2+1, y+y2+1, width-1, 64, 64, 64);
	root.DrawVertiLine(x+x2, y+y2, height, 128, 128, 128);
	root.DrawVertiLine(x+x2+1, y+y2+1, height-2, 64, 64, 64);
	root.DrawHorizLine(x+x2, y+y2+height, width+1, 255, 255, 255);
	root.DrawHorizLine(x+x2+1, y+y2+height-1, width-1, 212, 208, 200);
	root.DrawVertiLine(x+x2+width, y+y2, height, 255, 255, 255);
	root.DrawVertiLine(x+x2+width-1, y+y2+1, height-1, 212, 208, 200);
	root.Fillrect(x+x2+2, y+y2+2, width-3, height-3, 255, 255, 255);
	*/
...
void combobox::Draw(int x2, int y2) {
	...
	root.DrawHorizLine(x+x2+width-18, y+y2+2, 16, 212, 208, 200);
	root.DrawVertiLine(x+x2+width-18, y+y2+2, 18, 212, 208, 200);
	root.DrawHorizLine(x+x2+width-18, y+y2+height-2, 17, 64, 64, 64);
	root.DrawVertiLine(x+x2+width-2, y+y2+2, 18, 64, 64, 64);
	root.DrawHorizLine(x+x2+width-17, y+y2+3, 13, 255, 255, 255);
	root.DrawVertiLine(x+x2+width-17, y+y2+3, 15, 255, 255, 255);
	root.DrawVertiLine(x+x2+width-3, y+y2+3, 17, 128, 128, 128);
	root.DrawHorizLine(x+x2+width-17, y+y2+height-3, 15, 128, 128, 128);
	root.Fillrect(x+x2+width-16, y+y2+4, 13, 15, 212, 208, 200);
	root.DrawHorizLine(x+x2+width-14, y+y2+9, 7, 0, 0, 0);
	root.DrawHorizLine(x+x2+width-13, y+y2+10, 5, 0, 0, 0);
	root.DrawHorizLine(x+x2+width-12, y+y2+11, 3, 0, 0, 0);
	root.DrawHorizLine(x+x2+width-11, y+y2+12, 1, 0, 0, 0);
	*/
...
/*	strcpy(items[0], "Item1");
	...
	strcpy(items[3], "Item4");
	strcpy(items[4], "Item5");
	strcpy(items[5], "Item6");
	strcpy(items[6], "Item7");
	strcpy(items[7], "Item8");
	strcpy(items[8], "Item9");
	strcpy(items[9], "Item10");
	strcpy(items[10], "Item11");*/
...
void listbox::Draw(int x2, int y2) {
	...
	root.DrawVertiLine(x+x2+width-1, y+y2+1, height-1, 212, 208, 200);
	root.Fillrect(x+x2+2, y+y2+2, width-3, height-3, 255, 255, 255);
	root.Fillrect(x+x2+1, y+y2+2+(selectedItem*FONT_HEIGHT), width-1, FONT_HEIGHT+2, 10, 36, 106);
	*/
...
void taskbar::Draw(int x2, int y2) {
	...
	root.Fillrect(0, SCREENWIDTH-28, SCREENWIDTH, 28, 238, 237, 223);
	root.DrawHorizLine(0, SCREENWIDTH-29, SCREENWIDTH, 255, 255, 255);
	root.DrawHorizLine(0, SCREENWIDTH-30, SCREENWIDTH, 164, 164, 158);
...
void window::drawwindow() {			
		if (invisible != 1) {		
			...
			/*root.DrawHorizLine(x,y,width,212,208,200);
			root.DrawHorizLine(x+1,y+1,width-2,255,255,255);
			root.DrawVertiLine(x,y,height-1,212,208,200);
			root.DrawVertiLine(x+1,y+1,height-2,255,255,255);
			root.DrawHorizLine(x+1,y+height-2,width-1,128,128,128);
			root.DrawHorizLine(x,y+height-1,width,64,64,64);
			root.DrawVertiLine(x+width,y,height,64,64,64);
			root.DrawVertiLine(x+width-1,y+1,height-2,128,128,128);
			root.Fillrect(x+2,y+2,width-3,height-4,212,208,200);
			root.Fillrect(x+3,y+3,width-6,18,10,36,106);
			putstringc(title, x+5, y+4, 255, 255, 255, offscreen);*/
		}
}
...
extern "C"
void dispatch_keyboardevents(char *s) {
	int i,k;
...
void dispatch_mouseevents(int mousex, int mousey, char leftbuttondown) {
	...
	if ((leftbuttondown == 1) && (wasdrag == 0)) {
			...
			for (i=Shell.windowscount-1;i>=0;i--) {
				 ...
				 if (Shell.windows[wnd]->invisible != 1) {	// $$$ Taskbar Window is invisible
...
void SHinitShell(char *message, char *title, int width, int height) {
	...
	w2->createwindow("", SCREENWIDTH, SCREENWIDTH);
	w2->invisible=1;
	w2->controlscount=0;
	w2->handle=1;
	w2->z=1;
	...
	tb->createtaskbar("TASKBAR", 0, SCREENWIDTH-30, SCREENWIDTH, 30);
	tb->handle=0;
	tb->windowHandle=1;
	tb->refresh=1;
