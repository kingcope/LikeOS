...
void txtCtrl_Update(struct WND *ctrl, struct WND *window, unsigned char uselfb, unsigned fromKbdDriver) {
	...
	if (fromKbdDriver == 0) {
		bitmap=(unsigned long*)kalloc(width*height*3);
	...
	strcpy(buf, txtctrl->keyboard_buffer);
	p2=truncate_for_breaks(txtctrl, buf, (height-FONT_HEIGHT)/FONT_HEIGHT);
	//p2=buf;
	...
	while (1) {	
		...
		*p=0;
		if ((fin==0) && (*(p+1)==0) && (fromKbdDriver == 1)){
			...
			command_callback = txtctrl->command_callback;
	...
	kfree(buf);
	if (fromKbdDriver == 0) kfree(bitmap);
...
void addTextControl(struct WND *window, int x, int y, int width, int height, void *callback) {
	...
	struct textcontrol_t *textcontrol_struct=(struct textcontrol_t*)kalloc(sizeof(struct textcontrol_t));
	unsigned long i=0;
	...
	textcontrol_struct->keyboard_buffer=(unsigned char*) kalloc(65535);
...
void init_textCtrl() {
	scrolltop=(unsigned char*) kalloc(cdiso_getfilesize("SCLTOP.RAW"));
	cdiso_readfile("SCLTOP.RAW", scrolltop);
		
	scrollbottom=(unsigned char*) kalloc(cdiso_getfilesize("SCLBTM.RAW"));
